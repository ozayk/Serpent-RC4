...
if(argc !=4)
{
printf("Usage: serpent <key> <keylebgth> <filename>\n");
return 1;
}
else
{
...
unsigned char state[256], stream[528];
int len=528,idx,r1,r2; 
int keylen = (int)argv[2];

//KSA
ksa(state, key, keylen); 

FILE *fp  = fopen(argv[3],"r");
FILE *fp1 = fopen("decrypted.txt","w");
...
for(j = 0; j <= lp; j++)
{
fread(&data_buf[0], 4, 1, fp); 
fread(&data_buf[1], 4, 1, fp); 
fread(&data_buf[2], 4, 1, fp); 
fread(&data_buf[3], 4, 1, fp); 
...
memcpy(&w,stream_ptr,4);
stream_ptr += 4;
memcpy(&x,stream_ptr,4);
stream_ptr += 4;
memcpy(&y,stream_ptr,4);
stream_ptr += 4;
memcpy(&z,stream_ptr,4);
stream_ptr += 4;
k_get(r1,w,x,y,z);

}

//printf("data_buf[0] : %lx\n", data_buf[0]);
//printf("data_buf[1] : %lx\n", data_buf[1]);
//printf("data_buf[2] : %lx\n", data_buf[2]);
//printf("data_buf[3] : %lx\n\n", data_buf[3]);

encrypt(data_buf, enc_buf);
//printf("enc_buf[0] : %lx\n", enc_buf[0]);
//printf("enc_buf[1] : %lx\n", enc_buf[1]);
//printf("enc_buf[2] : %lx\n", enc_buf[2]);
//printf("enc_buf[3] : %lx\n\n", enc_buf[3]);

fwrite(&enc_buf[0], 4, 1, fp2); 
fwrite(&enc_buf[1], 4, 1, fp2); 
fwrite(&enc_buf[2], 4, 1, fp2); 
fwrite(&enc_buf[3], 4, 1, fp2); 



decrypt(enc_buf, dec_buf);
//printf("dec_buf[0] : %lx\n", dec_buf[0]);
//printf("dec_buf[1] : %lx\n", dec_buf[1]);
//printf("dec_buf[2] : %lx\n", dec_buf[2]);
//printf("dec_buf[3] : %lx\n", dec_buf[3]);

fwrite(&dec_buf[0], 4, 1, fp1); 
fwrite(&dec_buf[1], 4, 1, fp1); 
fwrite(&dec_buf[2], 4, 1, fp1); 
fwrite(&dec_buf[3], 4, 1, fp1); 
}
